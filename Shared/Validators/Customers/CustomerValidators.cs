...
using PBL3.Shared.DTOs.Customers; // Nhập (import) namespace PBL3.Shared.DTOs.Customers để sử dụng các thành phần bên trong.
using System; // Nhập thư viện hệ thống cơ bản.

namespace PBL3.Shared.Validators.Customers // Định nghĩa namespace PBL3.Shared.Validators.Customers quản lý cấu trúc code truyền tải và validator.
{
    public class CreateCustomerRequestValidator : AbstractValidator<CreateCustomerRequest> // Định nghĩa lớp kiểm định dữ liệu CreateCustomerRequestValidator kế thừa từ AbstractValidator cho CreateCustomerRequest.
    {
        public CreateCustomerRequestValidator() // Thực thi dòng lệnh nghiệp vụ.
        {
            RuleFor(x => x.FullName) // Bắt đầu thiết lập quy tắc kiểm tra dữ liệu cho thuộc tính FullName.
                ...
                .MaximumLength(100).WithMessage("Họ tên không được vượt quá 100 ký tự."); // Ràng buộc độ dài chuỗi tối đa không được vượt quá 100 ký tự.
    ...
    public class UpdateCustomerRequestValidator : AbstractValidator<UpdateCustomerRequest> // Định nghĩa lớp kiểm định dữ liệu UpdateCustomerRequestValidator kế thừa từ AbstractValidator cho UpdateCustomerRequest.
    {
        public UpdateCustomerRequestValidator() // Thực thi dòng lệnh nghiệp vụ.
        {
            RuleFor(x => x.FullName) // Bắt đầu thiết lập quy tắc kiểm tra dữ liệu cho thuộc tính FullName.
                .NotEmpty().WithMessage("Họ tên không được để trống.") // Ràng buộc kiểm tra thuộc tính không được để trống (Not Empty).
                .MaximumLength(100).WithMessage("Họ tên không được vượt quá 100 ký tự."); // Ràng buộc độ dài chuỗi tối đa không được vượt quá 100 ký tự.
    ...
    public class RegisterCustomerRequestValidator : AbstractValidator<RegisterCustomerRequest> // Định nghĩa lớp kiểm định dữ liệu RegisterCustomerRequestValidator kế thừa từ AbstractValidator cho RegisterCustomerRequest.
    {
        public RegisterCustomerRequestValidator() // Thực thi dòng lệnh nghiệp vụ.
        {
            RuleFor(x => x.FullName) // Bắt đầu thiết lập quy tắc kiểm tra dữ liệu cho thuộc tính FullName.
                .NotEmpty().WithMessage("Họ tên không được để trống.") // Ràng buộc kiểm tra thuộc tính không được để trống (Not Empty).
                .MaximumLength(100).WithMessage("Họ tên không được vượt quá 100 ký tự."); // Ràng buộc độ dài chuỗi tối đa không được vượt quá 100 ký tự.
            ...
            RuleFor(x => x.Password) // Bắt đầu thiết lập quy tắc kiểm tra dữ liệu cho thuộc tính Password.
                ...
                .Matches(@"[A-Z]").WithMessage("Mật khẩu phải chứa ít nhất một chữ hoa.") // Thiết lập thông báo lỗi khi kiểm tra thất bại: 'Mật khẩu phải chứa ít nhất một chữ hoa.'.
                .Matches(@"[a-z]").WithMessage("Mật khẩu phải chứa ít nhất một chữ thường.") // Thiết lập thông báo lỗi khi kiểm tra thất bại: 'Mật khẩu phải chứa ít nhất một chữ thường.'.
                .Matches(@"[0-9]").WithMessage("Mật khẩu phải chứa ít nhất một chữ số."); // Thiết lập thông báo lỗi khi kiểm tra thất bại: 'Mật khẩu phải chứa ít nhất một chữ số.'.

            RuleFor(x => x.ConfirmPassword) // Bắt đầu thiết lập quy tắc kiểm tra dữ liệu cho thuộc tính ConfirmPassword.
                .Equal(x => x.Password).WithMessage("Mật khẩu xác nhận không khớp."); // Thiết lập thông báo lỗi khi kiểm tra thất bại: 'Mật khẩu xác nhận không khớp.'.
        }
    }
}
